...
function Button({ offset, children }) {
  return (
    <button className="button" style={{ '--offset': `100px` }}>
      {children}
    </button>
  );
}
...
The entirety of this website is styled using these techniques, so if you want to see an example of how this applies to some real components, take a gander at the [source code](https://github.com/ShyamMW/portfolio).
